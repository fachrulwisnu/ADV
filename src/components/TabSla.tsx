...
export function TabSla({
  ...
  // Modal State
  const [drillDownModal, setDrillDownModal] = useState<"avg" | "2026" | "notAchieved" | "multiStage" | "yoy2025" | "yoy2026" | null>(null);
  ...
  const activeDevSlaProjs = useMemo(() => {
    return filteredProjects.filter(p => p["DEV SLA"] === "Achieved" || p["DEV SLA"] === "Not Achieved");
  }, [filteredProjects]);
  ...
  const activeDevSlaProjs2026 = useMemo(() => {
    return filteredProjects.filter(p => {
      ...
      return yr === "2026" && (p["DEV SLA"] === "Achieved" || p["DEV SLA"] === "Not Achieved");
    });
  }, [filteredProjects]);
  ...
  const activeDevSlaProjs2025 = useMemo(() => {
    return allProjects.filter(p => {
      ...
      return isMonthInRange(mIdx, startIdx, endIdx) && (p["DEV SLA"] === "Achieved" || p["DEV SLA"] === "Not Achieved");
    });
  ...
  // Not Achieved in selected range
  const devSlaNotAchievedProjs = useMemo(() => {
    return filteredProjects.filter(p => p["DEV SLA"] === "Not Achieved");
  }, [filteredProjects]);
  ...
  // SLA Stage Performance Horizontal metrics (Dynamic recalculation)
  const dynamicSlaStageSummary = useMemo(() => {
    const calculateStagePctDynamic = (key: string) => {
      const evaluated = filteredProjects.filter(p => p[key] === "Achieved" || p[key] === "Not Achieved");
      if (evaluated.length === 0) return 0;
      const achieved = evaluated.filter(p => p[key] === "Achieved").length;
      return Math.round((achieved / evaluated.length) * 100);
    };

    return [
      { stage: "FSD", pct: calculateStagePctDynamic("FSD SLA") },
      { stage: "DEV", pct: calculateStagePctDynamic("DEV SLA") },
      { stage: "SIT", pct: calculateStagePctDynamic("SIT SLA") },
      { stage: "UAT", pct: calculateStagePctDynamic("UAT SLA") },
      { stage: "LIVE", pct: calculateStagePctDynamic("Live SLA") },
    ];
  }, [filteredProjects]);
  ...
  const dynamicRootCause = useMemo(() => {
    ...
    return Object.keys(counts).map(k => ({
      label: k,
      value: counts[k],
      color: colors[k] || '#A855F7'
    })).sort((a, b) => b.value - a.value);
  }, [filteredProjects]);

  // YoY Growth Trend Comparison Bar Chart (strictly 2025 vs 2026 like-for-like)
  const yoyCompareData = useMemo(() => {
    const projs2025 = allProjects.filter(p => {
      const yr = String(p._year || p["Year"]);
      if (yr !== "2025") return false;
      const mIdx = getProjMonthIdx(p);
      return isMonthInRange(mIdx, startIdx, endIdx);
    });
    const achieved25 = projs2025.filter(p => p["DEV SLA"] === "Achieved").length;
    const notAchieved25 = projs2025.filter(p => p["DEV SLA"] === "Not Achieved").length;

    const projs2026 = allProjects.filter(p => {
      const yr = String(p._year || p["Year"]);
      if (yr !== "2026") return false;
      const mIdx = getProjMonthIdx(p);
      return isMonthInRange(mIdx, startIdx, endIdx);
    });
    const achieved26 = projs2026.filter(p => p["DEV SLA"] === "Achieved").length;
    const notAchieved26 = projs2026.filter(p => p["DEV SLA"] === "Not Achieved").length;

    return [
      { year: "2025", achieved: achieved25, not: notAchieved25, projects: projs2025 },
      { year: "2026", achieved: achieved26, not: notAchieved26, projects: projs2026 }
    ];
  }, [allProjects, startIdx, endIdx]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
          <Card
            ...
            sub="Pembagian kategori penyebab kegagalan SLA berdasarkan filter rentang waktu berjalan"
            padding="p-6"
            className="h-full"
          >
            <div className="flex flex-col sm:flex-row items-center gap-6 justify-center h-full">
              ...
              <div className="flex-shrink-0">
                <PieChart segments={dynamicRootCause} size={150} />
              </div>
                ...
                {dynamicRootCause.map((rc, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="flex items-center gap-2 font-bold text-gray-500 truncate max-w-[140px]">
                        <span
                ...
                {paginatedPhaseDelays.map((row, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 px-3 min-w-[180px] whitespace-normal break-words font-medium text-gray-600">
                        {row.reason}
                      </td>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-2xl w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-250">
            ...
            <div className="px-6 py-4.5 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
              <div>
                <h3 className="text-sm font-extrabold text-gray-900 font-display flex items-center gap-1.5">
                  <Icon name="grid" className="w-4 h-4 text-blue-600" />
                      ...
                      return (
                        <tr key={idx} className="hover:bg-gray-55/20 transition-colors">
                          ...
                          <td className="py-3 px-3 text-right">
                            <span className="inline-block px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-50 text-amber-700 border border-amber-100 whitespace-nowrap">
                              {p["Milestone"] || "In Progress"}
                            </span>
                          </td>
                        </tr>
                      );
